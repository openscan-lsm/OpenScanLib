...
OSc_RichError *OSc_Setting_GetValueType(OSc_Setting *setting,
                                        OSc_ValueType *valueType) {
...
OSc_Setting_GetNumericConstraintType(OSc_Setting *setting,
    ...
    *constraintType = OSc_ValueConstraint_None;
    ...
    errCode =
        setting->impl->GetNumericConstraintType(setting, &dConstraintType);
    if (errCode) {
        return OScInternal_Error_RetrieveFromSetting(setting, errCode);
    }
...
void OSc_Setting_SetInvalidateCallback(OSc_Setting *setting,
...
OSc_RichError *OSc_Setting_SetStringValue(OSc_Setting *setting,
                                          const char *value) {
...
OSc_RichError *OSc_Setting_GetInt32Value(OSc_Setting *setting,
                                         int32_t *value) {
...
OSc_RichError *OSc_Setting_GetInt32ContinuousRange(OSc_Setting *setting,
                                                   int32_t *min,
                                                   int32_t *max) {
...
OSc_RichError *OSc_Setting_GetInt32DiscreteValues(OSc_Setting *setting,
                                                  int32_t **values,
                                                  size_t *count) {
    if (!setting || !count) {
    ...
    if (setting->i32DiscreteValues == NULL) {
        ...
        if (vs) {
            ...
            for (size_t i = 0; i < c; ++i) {
                setting->i32DiscreteValues[i] =
                    (int32_t)OScInternal_NumArray_At(vs, i);
            }
...
OSc_RichError *OSc_Setting_GetFloat64Value(OSc_Setting *setting,
                                           double *value) {
    OScDev_Error errCode = setting->impl->GetFloat64(setting, value);
    return OScInternal_Error_RetrieveFromSetting(setting, errCode);
}

OSc_RichError *OSc_Setting_SetFloat64Value(OSc_Setting *setting,
                                           double value) {
    // TODO Should we validate the value here?
...
OSc_RichError *OSc_Setting_GetFloat64ContinuousRange(OSc_Setting *setting,
                                                     double *min,
                                                     double *max) {
...
OSc_RichError *OSc_Setting_GetFloat64DiscreteValues(OSc_Setting *setting,
                                                    double **values,
                                                    size_t *count) {
    if (!setting || !count) {
        return OScInternal_Error_IllegalArgument();
...
OSc_RichError *OSc_Setting_GetEnumValue(OSc_Setting *setting,
                                        uint32_t *value) {
...
OSc_RichError *OSc_Setting_GetEnumNumValues(OSc_Setting *setting,
                                            uint32_t *count) {
    OScDev_Error errCode = setting->impl->GetEnumNumValues(setting, count);
    return OScInternal_Error_RetrieveFromSetting(setting, errCode);
}

OSc_RichError *OSc_Setting_GetEnumNameForValue(OSc_Setting *setting,
                                               uint32_t value, char *name) {
    OScDev_Error errCode =
        setting->impl->GetEnumNameForValue(setting, value, name);
    return OScInternal_Error_RetrieveFromSetting(setting, errCode);
}

OSc_RichError *OSc_Setting_GetEnumValueForName(OSc_Setting *setting,
                                               uint32_t *value,
                                               const char *name) {
    OScDev_Error errCode =
        setting->impl->GetEnumValueForName(setting, value, name);
    return OScInternal_Error_RetrieveFromSetting(setting, errCode);
}
...
static OScDev_Error
DefaultGetNumericConstraint(OSc_Setting *setting,
...
static OScDev_Error DefaultGetInt32Range(OSc_Setting *setting, int32_t *min,
                                         int32_t *max) {
    *min = *max = 0;
    if (setting->valueType != OSc_ValueType_Int32)
        return OScDev_Error_Wrong_Value_Type;
...
static OScDev_Error DefaultGetInt32DiscreteValues(OSc_Setting *setting,
...
static OScDev_Error DefaultGetFloat64Range(OSc_Setting *setting, double *min,
                                           double *max) {
    *min = *max = 0.0;
    if (setting->valueType != OSc_ValueType_Float64)
        return OScDev_Error_Wrong_Value_Type;
...
static OScDev_Error DefaultGetFloat64DiscreteValues(OSc_Setting *setting,
                                                    OScDev_NumArray **values) {
    *values = NULL;
    if (setting->valueType != OSc_ValueType_Float64)
        return OScDev_Error_Wrong_Value_Type;
...
static OScDev_Error DefaultGetEnumNumValues(OSc_Setting *setting,
                                            uint32_t *count) {
    *count = 1;
    if (setting->valueType != OSc_ValueType_Enum)
        return OScDev_Error_Wrong_Value_Type;
...
static OScDev_Error DefaultGetEnumValueForName(OSc_Setting *setting,
                                               uint32_t *value,
                                               const char *name) {
    value = 0;
...
static void DefaultRelease(OSc_Setting *setting) { (void)setting; }

OSc_RichError *
OScInternal_Setting_Create(OScDev_ModuleImpl *modImpl, OSc_Setting **setting,
                           const char *name, OSc_ValueType valueType,
...
OSc_RichError *
OSc_Setting_NumericConstraintRange(OSc_Setting *setting,
                                   OSc_ValueConstraint *constraintType) {
    (void)setting;
...
OSc_RichError *OSc_Setting_NumericConstraintDiscreteValues(
    OSc_Setting *setting, OSc_ValueConstraint *constraintType) {
    (void)setting;
