...
static OScDev_Error
GetPixelRateConstraintType(OScDev_Setting *setting,
                           OScDev_ValueConstraint *constraint) {
...
static OScDev_Error GetPixelRateDiscreteValues(OScDev_Setting *setting,
                                               OScDev_NumArray **values) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetPixelRates(tmpl);
    if (!OScInternal_NumRange_IsDiscrete(range)) {
        OScInternal_Error_ReturnAsCode(
            OScInternal_Error_WrongConstraintType());
    }
...
static OScDev_Error GetPixelRateRange(OScDev_Setting *setting, double *min,
                                      double *max) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetPixelRates(tmpl);
    if (OScInternal_NumRange_IsDiscrete(range)) {
        return OScInternal_Error_ReturnAsCode(
            OScInternal_Error_WrongConstraintType());
    }
...
static OScDev_Error
GetResolutionConstraintType(OScDev_Setting *setting,
                            OScDev_ValueConstraint *constraint) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetResolutions(tmpl);
    if (OScInternal_NumRange_IsDiscrete(range)) {
...
static OScDev_Error GetResolutionDiscreteValues(OScDev_Setting *setting,
                                                OScDev_NumArray **values) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetResolutions(tmpl);
    if (!OScInternal_NumRange_IsDiscrete(range)) {
...
static OScDev_Error GetResolutionRange(OScDev_Setting *setting, int32_t *min,
                                       int32_t *max) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetResolutions(tmpl);
    if (OScInternal_NumRange_IsDiscrete(range)) {
...
static OScDev_NumRange *GetZooms(OSc_AcqTemplate *tmpl) {
    ...
    OScDev_NumRange *range =
        OScInternal_NumRange_Intersection(scannerRange, maxRange);
...
static OScDev_Error GetZoomConstraintType(OScDev_Setting *setting,
                                          OScDev_ValueConstraint *constraint) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetZooms(tmpl);
    if (OScInternal_NumRange_IsDiscrete(range)) {
...
static OScDev_Error GetZoomDiscreteValues(OScDev_Setting *setting,
                                          OScDev_NumArray **values) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetZooms(tmpl);
    if (!OScInternal_NumRange_IsDiscrete(range)) {
...
static OScDev_Error GetZoomRange(OScDev_Setting *setting, double *min,
                                 double *max) {
    OSc_AcqTemplate *tmpl = OScInternal_Setting_GetImplData(setting);
    OScDev_NumRange *range = GetZooms(tmpl);
    if (OScInternal_NumRange_IsDiscrete(range)) {
...
static OScDev_Error IsMagnificationWritable(OScDev_Setting *setting,
                                            bool *writable) {
    (void)setting;
    *writable = false;
    return OScDev_OK;
...
OSc_RichError *OSc_AcqTemplate_GetPixelRateSetting(OSc_AcqTemplate *tmpl,
                                                   OSc_Setting **setting) {
    if (!tmpl)
        return OScInternal_Error_IllegalArgument();
...
OSc_RichError *OSc_AcqTemplate_GetResolutionSetting(OSc_AcqTemplate *tmpl,
                                                    OSc_Setting **setting) {
    if (!tmpl)
        return OScInternal_Error_IllegalArgument();
...
OSc_RichError *OSc_AcqTemplate_GetZoomFactorSetting(OSc_AcqTemplate *tmpl,
                                                    OSc_Setting **setting) {
    if (!tmpl)
        return OScInternal_Error_IllegalArgument();
...
OSc_RichError *OSc_AcqTemplate_GetMagnificationSetting(OSc_AcqTemplate *tmpl,
                                                       OSc_Setting **setting) {
    if (!tmpl)
        return OScInternal_Error_IllegalArgument();
...
OSc_RichError *OSc_AcqTemplate_SetROI(OSc_AcqTemplate *tmpl, uint32_t xOffset,
    ...
    if (!OScInternal_Device_IsROIScanSupported(
            OSc_LSM_GetScannerDevice(tmpl->lsm))) {
        if (xOffset > 0 || yOffset > 0 || width != tmpl->resolution ||
            height != tmpl->resolution) {
...
OSc_RichError *OSc_AcqTemplate_GetROI(OSc_AcqTemplate *tmpl, uint32_t *xOffset,
                                      uint32_t *yOffset, uint32_t *width,
                                      uint32_t *height) {
