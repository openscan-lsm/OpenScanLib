...
OSc_RichError *OScInternal_Error_RetrieveFromDevice(OSc_Device *device,
    ...
    if (OScInternal_Device_SupportsRichErrors(device)) {
        ...
        return OScInternal_Error_CreateWithCode(
            OScInternal_Error_LegacyCodeDomain(), code,
            OScInternal_Error_MessageForLegacyCode(code));
    }
}
...
static const char *OScInternal_Error_MessageForLegacyCode(OScDev_Error code) {
    switch (code) {
    case OScDev_OK:
        OScInternal_LogError(
            NULL, "OScInternal_Error_MessageForLegacyCode: code cannot be 0.");
        abort();
...
void OScInternal_Error_FormatRecursive(OSc_RichError *error, char *buffer,
    ...
    if (RERR_Error_HasCause(error)) {
        ...
        OScInternal_Error_FormatRecursive(RERR_Error_GetCause(error), buffer,
                                          bufsize);
        len = strlen(buffer);
        buffer += len;
        bufsize -= len;
...
void OSc_Error_FormatRecursive(OSc_RichError *error, char *buffer,
                               size_t bufsize) {
