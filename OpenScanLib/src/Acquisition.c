...
OSc_RichError *OSc_Acquisition_Create(OSc_Acquisition **acq,
    ...
    *acq = calloc(1, sizeof(OSc_Acquisition));
    ...
    OScInternal_Device_GetNumberOfChannels((*acq)->detectorDevice,
                                           &(*acq)->numberOfChannels);
    OScInternal_Device_GetBytesPerSample((*acq)->detectorDevice,
                                         &(*acq)->bytesPerSample);
...
OSc_RichError *OSc_Acquisition_Arm(OSc_Acquisition *acq) {
    ...
    if (acq->detectorDevice != acq->clockDevice &&
        ...
        if (OSc_CHECK_ERROR(
                err, OScInternal_Device_Arm(acq->detectorDevice, acq))) {
