...
static inline struct DevicePrivateData *
GetSettingDeviceData(OScDev_Setting *setting) {
    return (struct DevicePrivateData *)OScDev_Device_GetImplData(
...
static bool WaitForSignal(const char *signalName, bool *cancelFlag,
                          CRITICAL_SECTION *mutex) {
    for (;;) {
        bool cancel;
...
static OScDev_Error SimulateImage(OScDev_Device *device,
...
static DWORD WINAPI AcquisitionLoop(void *param) {
    ...
    if (GetData(device)->useScanner) {
        if (!WaitForSignal("scanner.signal",
                           &GetData(device)->acquisition.stopRequested,
                           &GetData(device)->acquisition.mutex))
            canceled = true;
    }

    if (!canceled && GetData(device)->useDetector) {
        if (!WaitForSignal("detector.signal",
                           &GetData(device)->acquisition.stopRequested,
                           &GetData(device)->acquisition.mutex))
            canceled = true;
    }
...
OScDev_Error WaitForAcquisitionToFinish(OScDev_Device *device) {
    ...
    EnterCriticalSection(mutex);
    while (GetData(device)->acquisition.clockRunning ||
           GetData(device)->acquisition.scannerRunning ||
...
OScDev_Error IsAcquisitionRunning(OScDev_Device *device, bool *isRunning) {
    EnterCriticalSection(&(GetData(device)->acquisition.mutex));
    *isRunning = GetData(device)->acquisition.clockRunning ||
                 GetData(device)->acquisition.scannerRunning ||
                 GetData(device)->acquisition.detectorRunning;
    LeaveCriticalSection(&(GetData(device)->acquisition.mutex));
    return OScDev_OK;
...
static OScDev_Error EnumerateInstances(OScDev_PtrArray **devices) {
    *devices = OScDev_PtrArray_Create();
    ...
    struct DevicePrivateData *data =
        calloc(1, sizeof(struct DevicePrivateData));
...
static OScDev_Error GetPixelRates(OScDev_Device *device,
...
static OScDev_Error GetResolutions(OScDev_Device *device,
...
static OScDev_Error GetNumberOfChannels(OScDev_Device *device,
                                        uint32_t *nChannels) {
    (void)device;
    *nChannels = 1;
    return OScDev_OK;
...
static OScDev_Error GetBytesPerSample(OScDev_Device *device,
...
static OScDev_Error MakeSettings(OScDev_Device *device,
    ...
    *settings = OScDev_PtrArray_Create();
    ...
    if (OScDev_CHECK(err,
                     OScDev_Setting_Create(&lineDelay, "Error on start",
    ...
    EnterCriticalSection(&(GetData(device)->acquisition.mutex));
    {
        if (GetData(device)->simulatedErrorOnStart) {
            ...
            return OScDev_Error_ReturnAsCode(
                OScDev_Error_Create("simulated error"));
#endif
        }
...
#else
            return OScDev_Error_ReturnAsCode(
                OScDev_Error_Create("acquisition running"));
#endif
        }
