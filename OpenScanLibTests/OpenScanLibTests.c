...
static char *test_NumRange_Intersection(void) {
    ...
    mu_assert("continuous range expected",
              !OScInternal_NumRange_IsDiscrete(overlap));
    mu_assert("correct minimum expected",
              OScInternal_NumRange_Min(overlap) == 0.5);
    mu_assert("correct maximum expected",
              OScInternal_NumRange_Max(overlap) == 40.0);
